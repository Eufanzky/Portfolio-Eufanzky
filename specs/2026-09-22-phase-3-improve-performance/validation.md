...
- [x] After deploy: PSI mobile on production is recorded in `perf-baseline.md`. Target: 90 or higher, LCP 2.5 s or less, CLS 0.1 or less. If it misses, the gaps and next items are listed (step 20 stays open). Result: 94, 94, 94 (LCP 2.5 s, CLS 0–0.002).
...
- [x] Step 20 is marked as done after the production PSI run.
- [x] Merged into `main` through a PR after the user approved.
