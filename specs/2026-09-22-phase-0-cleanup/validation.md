...
- [ ] `npm run build` succeeds and emits the `vendor-react` and `vendor-motion` chunks, a separate async `react-three-fiber.esm-*.js` chunk, and `.gz`/`.br` files.
- [ ] `.eslintrc.cjs` turns rules off only through the overrides listed in `requirements.md`. There are no `eslint-disable` comments in `src/`.
...
- [ ] `src/components/canvas/Ball.jsx` no longer exists.
...
- [ ] The Network tab shows no `.gltf`/`.bin` model request and no `react-three-fiber.esm-*.js` chunk.
- [ ] No console errors.
- [ ] The Hero and About text still shows "I'm" correctly (the apostrophes were escaped).
...
- [ ] Roadmap steps 1–7 are marked as done in `specs/roadmap.md`.
